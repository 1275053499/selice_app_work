...
#import "TransferserviceController.h"
@interface TransferserviceController ()<UIScrollViewDelegate>{
...
@implementation TransferserviceController
...
//    mid悬浮块 UIbutton
    ...
    NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:@"点击上述按钮即同意《中国铺皇网商铺转让信息合同》及授权条款"];
...
-(void)creatBottomview{
    ...
    Bottomimageview.image    = [UIImage imageNamed:@"partners@2x"];
...
-(void)Clickshop{
    ...
    }else{  //[[YJLUserDefaults shareObjet]getObjectformKey:YJLTouchIDneed] no
...
-(void)joinBag{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        BagzrViewController *ctl =[[BagzrViewController alloc]init];
        self.hidesBottomBarWhenPushed = YES;//如果在push跳转时需要隐藏tabBar
        [self.navigationController pushViewController:ctl animated:YES];
        self.hidesBottomBarWhenPushed = YES;//1.并在push后设置self.hidesBottomBarWhenPushed=YES;2.这样back回来的时候，tabBar不会会恢复正常显示。
