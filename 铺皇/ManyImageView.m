...
//  小图的大小
#define ImageWidth ((KMainScreenWidth - 12 - 6) / 3)
//  全图的大小
#define AllImageWidth (KMainScreenWidth - 6)
...
- (void)frame:(CGRect)frame imageWithArr:(NSArray *)imageArr{
    ...
    UIScrollView *imageScroll = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, KMainScreenWidth, KMainScreenHeight)];
    imageScroll.backgroundColor = [UIColor blackColor];
    imageScroll.contentSize = CGSizeMake(KMainScreenWidth * imageArr.count, KMainScreenHeight);
    ...
    for (int i = 0; i < imageArr.count; i++) {
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(3 + KMainScreenWidth * i, (KMainScreenHeight - AllImageWidth) / 2, AllImageWidth, AllImageWidth)];
        
        [imageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",imageArr[i]]] placeholderImage:nil];//客服反馈图片
    ...
    pageControl.pageIndicatorTintColor = kTCColor(70, 70, 70);
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    return CGSizeMake((KMainScreenWidth - 2 * _widthMargin - (ImageWidthClearance * _number - 1)) / _number, (KMainScreenWidth - 2 * _widthMargin - (ImageWidthClearance * _number - 1)) / _number);
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    self.imageScroll.contentOffset = CGPointMake(KMainScreenWidth * tag, 0);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    int number = offSetX / KMainScreenWidth;
