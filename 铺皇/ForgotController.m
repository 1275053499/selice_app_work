...
- (void)viewDidLoad {
    ...
    NSLog(@"传值过来的电话号码：%@",self.user);
    NSLog(@"传值过来的电话号码：%@",self.userID);
...
-(void)newsureClick{
    ...
    NSLog(@"%@",params);
    [manager POST:ForgetTwopath parameters:params success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if ([rescode isEqualToString:@"200"]){//改账号的密码修改成功了
            NSLog(@"改账号的密码修改成功了");
            
            //  本地存数据 账号密码
            [[YJLUserDefaults shareObjet]saveObject:self.user forKey:YJLuser];
            [[YJLUserDefaults shareObjet]saveObject:newpassword.text forKey:YJLpassword];
            [[YJLUserDefaults shareObjet]saveObject:@"loginno" forKey:YJLloginstate];
        
//            NSUserDefaults *defaults      =     [NSUserDefaults standardUserDefaults];
//            [defaults setObject:self.user        forKey:@"user"];//账号
//            [defaults setObject:newpassword.text forKey:@"password"];//新密码
//            [defaults setObject:@"loginno"       forKey:@"loginstate"];//账号登录状态
//            [defaults synchronize];
            
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"温馨提示" message:@"密码修改成功" preferredStyle:UIAlertControllerStyleAlert];
        ...
        else if ([rescode isEqualToString:@"201"]){  //改账号的密码修改失败了
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"温馨提示" message:@"输入新密码居然是初始密码！" preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction *commitAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action){
                NSLog(@"输入新密码与原始密码一致");
        ...
        else if ([rescode isEqualToString:@"301"]){  //改账号的密码修改失败了
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"温馨提示" message:@"密码修改失败了" preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction *commitAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action){
                NSLog(@"密码修改失败了");
        ...
        else if ([rescode isEqualToString:@"309"]){  //改账号的密码修改失败了
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"温馨提示" message:@"账号不存在！" preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction *commitAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action){
                NSLog(@"密码修改失败了");
            }];
