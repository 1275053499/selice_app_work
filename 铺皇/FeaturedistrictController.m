...
@interface FeaturedistrictController ()<YJLMenuDataSource,YJLMenuDelegate,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>{
    int PHpage;
...
@implementation FeaturedistrictController
...
#pragma -mark   适配必要使用
    ...
    self.keywordstr = [[NSString alloc]init];
    self.keywordstr = @"";
    ...
    self.path           = [[NSString alloc]initWithFormat:@"&upid=00000&rent=00000&moneys=00000&area=00000&type=00000&views=00000&times=00000&keyword="];
    //    创建导航栏控件
    ...
    // 是否是第一次进入该处
    [self isdistricFirstCome];
}

#pragma  -mark 切换城市进行判断并删除上一个城市数据
-(void)changevalue{
    
    [[NSUserDefaults standardUserDefaults] setObject:@"NO" forKey:@"districisFirstCome"    ];
    [[NSUserDefaults standardUserDefaults] synchronize];
    [[SiftareaData       shareareaData    ]deletedareaData];

}

-(void)isdistricFirstCome{
    
    NSString       * isFirstCome       = [[NSUserDefaults standardUserDefaults] objectForKey:@"districisFirstCome"];
...
#pragma -mark 网络检测
        ...
        self.PHDataArr   = [[SiftareaData shareareaData] getAllDatas];
        NSLog(@"有%ld个", self.PHDataArr.count);
        if ( self.PHDataArr.count>0) {
#pragma -mark 网络检测
             [self.Districttableview reloadData];
            if ( self.PHDataArr.count % 10>0) {
                PHpage = (int) self.PHDataArr.count/10;
            }else{
                PHpage = (int) self.PHDataArr.count/10-1;
            }
...
#pragma  -mark下拉刷新获取网络数据
   ...
   self.Districttableview.mj_header = header;
    
    //#pragma  -mark上拉加载获取网络数据
    self.Districttableview.mj_footer=[MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
        NSLog(@"上拉刷新一下试试"           );
...
-(void)loadrentupData{
    ...
    PHpage = 0;
    [self.Districttableview.mj_footer  resetNoMoreData];
...
//            NSLog(@"可以拿到数据的");
            ...
            for (NSDictionary *dic in responseObject[@"values"]){
                ...
                //            添加到数据库
                [[SiftareaData shareareaData]addshoparea:model];
            ...
            self.Districttableview.separatorStyle = UITableViewCellSeparatorStyleSingleLine;
            [self.BGlab setHidden:YES];
        }else{
            
            [self.BGlab setHidden:NO];
            self.BGlab.text          = @"服务器开小差了，稍等~~";
            [self.Districttableview .mj_header endRefreshing];
...
//            NSLog(@"可以拿到数据的");
            ...
            [YJLHUD dismissWithDelay:1];
            [self.Districttableview.mj_footer endRefreshingWithNoMoreData];
        }
        // 主线程执行：
        dispatch_async(dispatch_get_main_queue(), ^{
            [self. Districttableview reloadData];
        ...
        [self.Districttableview .mj_footer endRefreshing];
...
-(UITableViewCell * )tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.Searchtableview) {
        ...
        if (indexPath.section==0) {
            ...
            //            NSLog(@"!!!!!%ld=????????%ld",PHDataArr.count,indexPath.row);
            Featuremodel *model = [ self.PHDataArr objectAtIndex:indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.Searchtableview)
    {
        ...
        {    //清除所有历史记录
            ...
            [self.Districttableview    setHidden:NO];
...
- (void)configKeyBoardRespond {
    ...
    __weak FeaturedistrictController *weakSelf = self;
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    ...
    [self.Districttableview   setHidden:YES];
...
-(NSString *)menu:(YJLMenu *)menu titleForRowAtIndexPath:(YJLIndexPath *)indexPath{
    ...
    if (indexPath.column  == 0) {
        ...
        return self.Regionaname[indexPath.row];
    ...
    if (indexPath.item >= 0)   //有二级菜单
    {
        ...
        switch (indexPath.column+1){
            case 1:{
                ...
                valuedistric1 = self.Regionaname[indexPath.row];
                valuedistric1id = self.Regionaid[indexPath.row];
                NSLog(@"获取值区域   = %@",valuedistric1);
                NSLog(@"获取值区域id = %@",valuedistric1id);
            }
                break;
...
#pragma -mark 显示当前点击多少项  那几项名称 id
        [self setup:valuedistric1id :valuetype2id :valuebrose3id :valuetime4id];
    }
}
...
-(void)setup:(NSString *)value1 :(NSString *)value2 : (NSString *)value3 :(NSString *)value4{
    ...
    _path = [NSString stringWithFormat:@"&upid=%@&rent=00000&moneys=00000&area=00000&type=%@&views=%@&times=%@&keyword=",value1,value2,value3,value4];
    NSLog(@"拼接字符串%@",_path);
    [self loadrentupData];
}
...
-(void)creattableview{
     ...
     self.Districttableview = [[UITableView alloc]initWithFrame:CGRectMake(0 , CGRectGetMaxY(_menu.frame), KMainScreenWidth, KMainScreenHeight-114)];
    self.Districttableview.delegate   = self;
    self.Districttableview.dataSource = self;
    [self.Districttableview setHidden:NO];
    self.Districttableview.backgroundColor = [UIColor clearColor];
    self.Districttableview.tableFooterView = [UIView new];
    self.Districttableview.separatorStyle  = UITableViewCellSeparatorStyleNone;
    [self.view addSubview:self.Districttableview];

    //    无数据的提示
    self.BGlab                   = [[UILabel alloc]init];
    [self.Districttableview addSubview:self.BGlab];
    ...
    [self.BGlab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.center.equalTo(self.Districttableview);
...
-(void)readNSUserDefaults{
    ...
    //读取数组NSArray类型的数据
    NSArray * myArray = [[NSUserDefaults standardUserDefaults] arrayForKey:@"myArray"];
