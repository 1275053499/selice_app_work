...
#define kTextSelectColor [UIColor colorWithRed:77/255.0 green:166/255.0 blue:214/255.0 alpha:1]
#define kTableViewHeight 350

typedef void(^complete)(void);

#pragma mark - YJLIndexPath
@implementation YJLIndexPath
...
+ (instancetype)indexPathWithColumn:(NSInteger)column row:(NSInteger)row {
    YJLIndexPath *path = [[self alloc] initWithColumn:column row:row];
    return path;
}
...
#pragma mark - YJLMenu
@interface YJLMenu () <UITableViewDataSource, UITableViewDelegate>
{
    struct {
...
- (void)setDataSource:(id<YJLMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numberOfColumn; i++)
    {
        ...
        if (!_isClickHaveItemValid && [_dataSource menu:self numberOfItemsInRow:0 inColumn:i] > 0 && _dataSourceFlag.numberOfItemsInRow) {
            titleString = [_dataSource menu:self titleForItemsInRowAtIndexPath:[YJLIndexPath indexPathWithColumn:i row:0 item:0]];
        }
        else {
            titleString = [_dataSource menu:self titleForRowAtIndexPath:[YJLIndexPath indexPathWithColumn:i row:0]];
        }
        
        CGPoint positionForTitle = CGPointMake(( i * 2 + 1) * numberOfTextLayer-10, self.height / 2);
...
- (CGSize)calculateTitleSizeWithString:(NSString *)string
{
    ...
    CGSize size = [string boundingRectWithSize:CGSizeMake(100, 0) options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:dic context:nil].size;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == _leftTableView) {
        if (_dataSourceFlag.titleForRowsAtIndexPath) {
            cell.textLabel.text = [_dataSource menu:self titleForRowAtIndexPath:[YJLIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
            if (_dataSourceFlag.imageNameForRowAtIndexPath) {
                NSString *imgName = [_dataSource menu:self imageNameForRowAtIndexPath:[YJLIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
            ...
            if (_dataSourceFlag.detailTextForRowAtIndexPath) {
                cell.detailTextLabel.text = [_dataSource menu:self detailTextForRowAtIndexPath:[YJLIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
            }else {
        ...
        if (_dataSourceFlag.titleForItemInRowAtIndexPath) {
            ...
            cell.textLabel.text = [_dataSource menu:self titleForItemsInRowAtIndexPath:[YJLIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
            if (_dataSourceFlag.imageNameForItemInRowAtIndexPath) {
                NSString *imgName = [_dataSource menu:self imageForItemsInRowAtIndexPath:[YJLIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
                if (imgName && imgName.length > 0) {
                    cell.imageView.image = [UIImage imageNamed:imgName];
                }else {
                    cell.imageView.image = nil;
            ...
            if (_dataSourceFlag.detailTextForItemInRowAtIndexPath) {
                cell.detailTextLabel.text = [_dataSource menu:self detailTextForItemsInRowAtIndexPath:[YJLIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
            }else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == _leftTableView) {
        ...
        if (isClickHaveItemValid && _delegate &&[_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            [_delegate menu:self didSelectRowAtIndexPath:[YJLIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
        }
    }else {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            [_delegate menu:self didSelectRowAtIndexPath:[YJLIndexPath indexPathWithColumn:_currentSelectedColumn row:[_currentSelectedRows[_currentSelectedColumn] integerValue] item:indexPath.item]];
        }
    }
}
...
- (void)selectDeafultIndexPath {
    [self selectIndexPath:[YJLIndexPath indexPathWithColumn:0 row:0]];
}
...
- (void)selectIndexPath:(YJLIndexPath *)indexPath {
    ...
    if (indexPath.item < 0 ) {
        ...
        if (!_isClickHaveItemValid && [_dataSource menu:self numberOfItemsInRow:indexPath.row inColumn:indexPath.column] > 0){
            title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[YJLIndexPath indexPathWithColumn:indexPath.column row:indexPath.row]];
            [_delegate menu:self didSelectRowAtIndexPath:[YJLIndexPath indexPathWithColumn:indexPath.column row:indexPath.row item:0]];
        }else {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[YJLIndexPath indexPathWithColumn:indexPath.column row:indexPath.row]];
...
- (void)backTapped:(UITapGestureRecognizer *)gesture {
    [self animateIndicator:_indicators[_currentSelectedColumn] background:_backGroundView tableView:_leftTableView title:_titles[_currentSelectedColumn] reverse:NO complecte:^{
        _isShow = NO;
         ...
         [[NSNotificationCenter defaultCenter] postNotificationName:@"ShowNO" object:nil userInfo:nil];
    }];
    NSLog(@"1321321321321321321312312321321");
}
...
- (BOOL)setMenuWithSelectedRow:(NSInteger)row {
    ...
    if (_dataSourceFlag.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:row inColumn:_currentSelectedColumn] > 0) {
        if (_isClickHaveItemValid) {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[YJLIndexPath indexPathWithColumn:_currentSelectedColumn row:row]];
...
- (void)setMenuWithSelectedItem:(NSInteger)item {
    ...
    title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[YJLIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedMenudRow item:item]];
    [self animateIndicator:_indicators[_currentSelectedColumn] background:_backGroundView tableView:_leftTableView title:title reverse:NO complecte:^{
        _isShow = NO;
    }];
}
