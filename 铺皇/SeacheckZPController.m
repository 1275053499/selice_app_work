...
#import "SeacheckZPController.h"
#import "ShopsrecruitModel.h"
#import "ShopsrecruitViewCell.h"
#import "ResumeXQController.h"
@interface SeacheckZPController ()<UITableViewDelegate,UITableViewDataSource>{
    int  PHpage;
    CGFloat  OffestY;
}
...
@implementation SeacheckZPController
...
-(void)loaddataUPtodown{
    ...
    NSString  * URL = [NSString stringWithFormat:@"%@?city=%@&keyword=%@&page=%d",HostmainSerach,self.Searchcity_ZP,self.Searchword_ZP,PHpage];
...
//            NSLog(@"可以拿到数据的");
            ...
            for (NSDictionary *dic in responseObject[@"data"][@"zp"]){
                ...
                model.CompanyJobname    = dic[@"name"];
                model.Companyname       = dic[@"descript"];
                model.CompanyArea       = dic[@"dityour"];
              ...
              NSLog(@" ZP加载后现在总请求到数据有%ld个",_PHDataArr.count);
        }
...
-(UITableViewCell * )tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ShopsrecruitModel *model = [_PHDataArr objectAtIndex:indexPath.row];
    cell.CompanyJobname.text =model.CompanyJobname;
    cell.CompanyTimers.text  = model.CompanyTimers;
    cell.Companyname.text    = model.Companyname;
    cell.CompanyArea.text    = model.CompanyArea;
    cell.CompanySuffer.text  = model.CompanySuffer;
    cell.Companyeducation.text = model.Companyeducation;
...
-(void)creatBacktitle{
    ...
    if (self.Searchword_ZP.length<1) {
        _titlelab.text          =@"店铺招聘";
    }else{
        _titlelab.text          =[NSString stringWithFormat:@"搜索:%@",[self.Searchword_ZP stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]]];
    }
