...
#import "ShopsleaseViewController.h"
#import "YJLMenu.h"
#import "JX_FourCell.h"
#import "JX_FourModel.h"
#import "CZdataBase.h"//数据缓存数据库
#import "Cityareamodel.h"//城市区域model
#import "Cityarea.h"//城市区域数据库
@interface ShopsleaseViewController ()<UITableViewDelegate,UITableViewDataSource,YJLMenuDelegate,YJLMenuDataSource,UINavigationControllerDelegate>{
        int  PHpage;
    YJLMenu         *yjlmenu;
...
@property (nonatomic , strong)UITableView   * Shopsleasetableview;//列表
...
@property (nonatomic, strong) NSArray       * Type;    //价格选店
...
@implementation ShopsleaseViewController
...
- (void)viewDidLoad
{
    ...
    self.path           = [[NSString alloc]initWithFormat:@"&search=00000&rent=00000&trade=00000&area=00000"];
...
-(void)ShowNO{
    ...
    [self.Shopsleasetableview setHidden:NO];
}
...
-(void)changevalue{
    ...
    [[NSUserDefaults standardUserDefaults] setObject:@"NO" forKey:@"CZisFirstCome"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    [[CZdataBase shareCZdataBase]deletedCZdata];
     NSLog(@"切换城市----出租的缓存的数据清理");
}

#pragma -mark 是否是第一次进入该处
-(void)isNewsFirstCome{
    
    NSString       * isFirstCome       = [[NSUserDefaults standardUserDefaults] objectForKey:@"CZisFirstCome"];
    NSLog(@"YES OR NO                                                  ===%@",isFirstCome);
    if (![isFirstCome isEqualToString:@"YES"])
    {   //NO
        NSLog(@"是第一次请求");
        ...
        self.PHDataArr = [[CZdataBase shareCZdataBase] getAllDatas];
...
- (void)refresh{
    ...
    self.Shopsleasetableview.mj_header = header;
    
#pragma  -mark上拉加载获取网络数据
    self.Shopsleasetableview.mj_footer=[MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
...
-(void)loaddataUPtodown{
    ...
    [self.Shopsleasetableview.mj_footer resetNoMoreData];
    NSLog(@"即将下拉刷新之前数组有%ld个数据",self.PHDataArr.count); 
    NSString  * URL = [NSString stringWithFormat:@"%@?cid=%@&page=%d%@",HostCZrentpath,_hostcityid,PHpage,_path];
    NSLog(@"下拉刷新请求入境：%@",URL);
...
//        NSLog(@"马上加载第%d页",PHpage);
    [YJLHUD showMyselfBackgroundColor:nil ForegroundColor:nil BackgroundLayerColor:nil message:@"加载中...."];
        ...
        NSString  * str = [NSString stringWithFormat:@"%@?cid=%@&page=%d%@",HostCZrentpath,_hostcityid,PHpage,_path];
        NSLog(@"上拉加载请求入境：%@",str);
...
//        适配一下区域的宽度 end
        ...
        JX_cell.Pricelab.text             = [NSString stringWithFormat:@"%@元／月",model.JX_rent];//店铺费
...
-(void)dealloc{
    ...
    [_Shopsleasetableview removeFromSuperview];
    _Shopsleasetableview = nil;
...
-(void)creatTopview{
    ...
    [self.ImageView setImage:[UIImage imageNamed:@"招商banner"]];
...
#pragma mark -创建tableview
    self.Shopsleasetableview = [[UITableView alloc]init];
    self.Shopsleasetableview.delegate = self;
    self.Shopsleasetableview.dataSource = self;
    self.Shopsleasetableview.backgroundColor = [UIColor whiteColor];
    self.Shopsleasetableview.tableFooterView = [UIView new];
    [self.view addSubview:self.Shopsleasetableview];
    [self.Shopsleasetableview mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [self.Shopsleasetableview addSubview:self.BGlab];
    ...
    [self.BGlab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.center.equalTo(self.Shopsleasetableview);
...
-(NSString *)menu:(YJLMenu *)menu titleForRowAtIndexPath:(YJLIndexPath *)indexPath{
    [self.Shopsleasetableview setHidden:YES];//重要的特效点1
...
- (void)menu:(YJLMenu *)menu didSelectRowAtIndexPath:(YJLIndexPath *)indexPath {
     [self.Shopsleasetableview setHidden:NO];//重要的特效点1
...
-(void)setup:(NSString *)value1 :(NSString *)value2 : (NSString *)value3 :(NSString *)value4{
    ...
    _path = [NSString stringWithFormat:@"&search=%@&rent=%@&trade=%@&area=%@",value1,value2,value3,value4];
    NSLog(@"拼接字符串%@",_path);
    [self loaddataUPtodown];
}
...
//    NSLog(@"将要开始拖拽，手指已经放在view上并准备拖动的那一刻===%f",offset);
    if (scrollView == self.Shopsleasetableview) {
        NSLog(@"2132132112321");
    }
}
...
//        NSLog(@"只要view有滚动=%f",offset);
        if (offset <= 0) {
            ...
            [self.Shopsleasetableview mas_remakeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo (self.view).with.offset(244);
                make.left.equalTo (self.view).with.offset(0);
                make.size.mas_equalTo(CGSizeMake(KMainScreenWidth, KMainScreenHeight-244));
            }];
