...
@interface ChooseserviceController ()<UIScrollViewDelegate>
{
...
-(void)creatMidview{
    ...
    [Midmainbtn setTitle:@"立即发布" forState:UIControlStateNormal];
    ...
    NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:@"点击上述按钮《中国铺皇网商铺选址信息合同》及授权条款"];
...
-(void)Midclick:(UIButton *)btn{
    ...
    NSLog(@"立即发布");
    ReleaseZSController *ctl =[[ReleaseZSController alloc]init];//套餐页面
    self.hidesBottomBarWhenPushed = YES;//如果在push跳转时需要隐藏tabBar
    [self.navigationController pushViewController:ctl animated:YES];
    self.hidesBottomBarWhenPushed = YES;//1.并在push后设置self.hidesBottomBarWhenPushed=YES;2.这样back回来的时候，tabBar不会会恢复正常显示。
...
-(void)Protocolclick:(UITapGestureRecognizer *)tap{
    ...
    ctl.url =@"https://ph.chinapuhuang.com/index.php/index/xz";//注册协议
    self.hidesBottomBarWhenPushed = YES;//如果在push跳转时需要隐藏tabBar
    [self.navigationController pushViewController:ctl animated:YES];
    self.hidesBottomBarWhenPushed = YES;//1.并在push后设置self.hidesBottomBarWhenPushed=YES;2.这样back回来的时候，tabBar不会会恢复正常显示。
}
